...
export const SECTION_HEADER_RE =
  /^(task|tool|format|constraints?|input|context|audience|success|examples?)\s*:\s*(.+)/im;

export const ALL_SECTION_HEADERS_RE =
  /^(task|tool|format|constraints?|input|context|audience|success|examples?)\s*:\s*(.+(?:\n(?!(?:task|tool|format|constraints?|input|context|audience|success|examples?)\s*:).+)*)/gim;

// Fenced code blocks
export const FENCED_CODE_RE = /```[\s\S]*?```/g;

// "given the following" anchors
export const GIVEN_FOLLOWING_RE =
  /given\s+the\s+following[^:]*:\s*([\s\S]+?)(?:\n\n|\n(?=[A-Z])|\s*$)/i;
